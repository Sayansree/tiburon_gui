...
using namespace std;
using namespace cv;
redflare::redflare(QWidget *parent) : QMainWindow(parent), ui(new Ui::redflare) {
  LOGO_PATH = ros::package::getPath("tiburon_gui") + "/utils/logo.jpeg";
  CONFIG_PATH = ros::package::getPath("tiburon_gui")+ "/config/redflare_config.dat";
...
redflare::~redflare() { delete ui; }

void redflare::hsv(int value,int index){HSV[index]=value;}
void redflare::channel(int index){ch_index=index;}
void redflare::h(int val){HSV[0]=val;}
void redflare::s(int val){HSV[1]=val;}
void redflare::v(int val){HSV[2]=val;}
void redflare::H(int val){HSV[3]=val;}
void redflare::S(int val){HSV[4]=val;}
void redflare::V(int val){HSV[5]=val;}
void redflare::opn(int val){S_op=val;}
void redflare::sob(int val){S_sob=val;}
void redflare::out_chan(int val){out_ch=val;}
void redflare::Threshold(int val){threshold=val;}
void redflare::wid(int val){width=val;}
void redflare::hei(int val){height=val;}

void redflare::feed(cv::Mat img2)
{
...
void redflare::channel_show(cv::Mat img)
{
